var Dijkstra= {
	...
	_updateCostIfLower : function(nodes, fromNode){
		...
		var reachablefromNode = nodes.filter(function(node){return node.parents.indexOf(fromNode.id)>-1;})
	...
	_visitAndRemove : function(nodes, visited, fromNodeId, updateStrategy, nextNodeStrategy){
		...
		var head = nodes.find(function(node){return node.id===fromNodeId;});
		nodes = nodes.filter(function(node){return node.id!==fromNodeId;});
	...
	_sortByCostToNode : function(nodeId, lhs, rhs){
		var lhsNode = lhs.find(function(node){ return node.id === nodeId });
		var rhsNode = rhs.find(function(node){ return node.id === nodeId });
	...
	longestPathPossible : function(graph){
	    ...
	    var rootWithLongestPathToNode = byRoot.length===0
		    ? []
